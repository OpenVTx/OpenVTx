...
**VTX mode LED indication** - Indicate current operating mode (band/channel/power) similar to TBS's approach. To disable comment ``LED_INDICATION_OF_VTX_MODE`` define in ``commons.h`` file before building the project.
...
The **VTX mode LED indication** indication logic:
