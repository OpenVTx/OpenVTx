...
// 1, 1 5mW
  ...
  vref_pin = gpio_out_setup(VREF, 0); // Power amp OFF
  vref_pin = gpio_out_setup(RTC_BIAS, 1);
  // vref_pin = gpio_out_setup(VREF, 1); // Power amp ON
  // vref_pin = gpio_out_setup(RTC_BIAS, 0);

  // outputPowerTimer = pwm_init(RTC_BIAS);
  // pwm_out_write(outputPowerTimer, 800);
  // pwm_out_write(outputPowerTimer, 255);
  // pwm_out_write(outputPowerTimer, 128);
...
void taget_loop(void)
{
  ...
  if (200 <= (now - temp)) {
    ...
    currentVpd = 85;
    // Serial_write((uint8_t)(currentVpd / 1000));  
    // Serial_write((uint8_t)(currentVpd % 1000) / 100);  
    // Serial_write((uint8_t)(currentVpd % 100) / 10);  
    // Serial_write((uint8_t)(currentVpd % 10));  
